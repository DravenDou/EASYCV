...
def is_bundled_typst_package_complete(
    bundled_path: pathlib.Path,
    typ_files: list[str],
) -> bool:
    """Return whether a bundled Typst package has all required files.

    Why:
        Some development checkouts intentionally omit vendored package files.
        This check allows optional bundled packages to be skipped without
        crashing compiler setup.

    Args:
        bundled_path: Path to the bundled package directory.
        typ_files: List of .typ filenames required for the package.

    Returns:
        True if typst.toml and all listed .typ files are present.
    """
    required_files = [bundled_path / "typst.toml", *[bundled_path / f for f in typ_files]]
...
def get_package_path() -> pathlib.Path:
    ...
    rendercv_typ_files = ["lib.typ"]
    rendercv_path = renderer_dir / "rendercv_typst"
    if not is_bundled_typst_package_complete(rendercv_path, rendercv_typ_files):
        message = (
            "Bundled Typst package `rendercv` is incomplete. Expected typst.toml and "
            "required .typ files under renderer/rendercv_typst."
        )
        raise RenderCVInternalError(message)

    install_bundled_typst_package(
        bundled_path=rendercv_path,
        package_name="rendercv",
        temp_dir=temp_dir,
        typ_files=rendercv_typ_files,
    )

    fontawesome_typ_files = [
        "lib.typ",
        "lib-impl.typ",
        "lib-gen-func.typ",
        "lib-gen-map.typ",
    ]
    fontawesome_path = renderer_dir / "typst_fontawesome"

    # If Font Awesome package files are not vendored, Typst can still resolve
    # @preview/fontawesome from its own package cache/registry.
    if is_bundled_typst_package_complete(fontawesome_path, fontawesome_typ_files):
        install_bundled_typst_package(
            bundled_path=fontawesome_path,
            package_name="fontawesome",
            temp_dir=temp_dir,
            typ_files=fontawesome_typ_files,
        )
